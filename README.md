...
- 실시간으로 물체를 검출·추적하고 3D 위치를 추정하며 (YOLO26-seg + DA3-Small depth)
...
첫 실행 시 모델 가중치(DA3-SMALL ~0.3GB, DA3METRIC-LARGE ~1.4GB, YOLO26n-seg)가
자동 다운로드됩니다. Rerun 뷰어 창이 자동으로 열립니다.
...
```
[실시간 계층 — 매 프레임, MPS]
  YOLO26n-seg + ByteTrack  → 객체 mask/추적
...
```bash
.venv/bin/python -m pytest tests/ -q          # 단위 테스트 (기하/보정/레지스트리)
...
- 10.8초 샘플 영상으로 개발됨 — 1~3분짜리 실내 촬영 영상으로 영속성/재등장 검증 권장.
