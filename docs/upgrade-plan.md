...
## Tier 4 — 루프 클로저 (예상 1주+, 위험 중간~높음) — ✅ 1차 완료 (2026-06-11)
...
> **결과 요약**: `loop.py`(detector + 3D-3D RANSAC Sim3 + pose graph) +
> 백엔드 워커 통합 + epoch별 voxel 지도 보정 구현. **설계 변경 1건**:
> 루프 검증을 DA3 멀티뷰 대신 ORB+depth 3D-3D RANSAC으로 (DA3 pose
> 신뢰성 문제 회피, Tier 2 소견 반영). 부메랑 영상 A/B에서 루프 수락
> 15쌍, 지도 이중상 −29%, 복귀 오차 개선. 튜닝 함정 3개(엣지별 스케일
> 가중, T_gl 합성 갱신, robust_sim3 클램프)와 v1 한계(GS 레이어 미보정,
> 객체 즉시 이동 미구현)는 `docs/benchmarks.md` Tier 4 섹션. 모든 보정은
> 기존 T_global_live 보간으로 무점프 반영. loop.enabled=false(기본)면
> 기존 동작과 동일 — 실내 재방문 실촬영 영상으로 추가 검증 권장.
