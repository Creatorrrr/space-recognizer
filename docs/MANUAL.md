...
| `--max-frames <N>` | 앞부분 N프레임만 처리하고 종료 (벤치마크/스모크용) |
| `--no-realtime` | 벽시계 페이싱 없이 모든 프레임을 처리 (오프라인 분석·검증용) |
| `--profile` | 10프레임마다 단계별 처리 시간(ms) 출력 (성능 진단용) |
| `--perf-log <CSV>` | 프레임별 stage timing, stutter, 품질 지표를 CSV로 저장 |
| `--stutter-threshold-ms <ms>` | `[stutter]`로 표시할 loop_total 임계값 (기본 100ms) |
| `--runtime-profile realtime` | OAK metric replay용 저지연 설정 overlay 적용 |
| `--no-viz` | Rerun 로깅을 끄고 계산 상한선 측정 |
| `--no-backend` | 백엔드 재구성을 끄고 live loop 상한선 측정 |
...
# OAK replay 끊김 진단: 프레임별 CSV와 stutter breakdown 저장
.venv/bin/python -m spacerec.main \
...
runtime_profile: null             # null/quality 또는 realtime
...
backend:
  enabled: true
  live_apply: true                # false면 backend 결과를 종료/idle 때 반영
...
objects:
  ...
  appearance_keyframes_only: false # true면 키프레임에서만 외형 임베딩
  appearance_every_n_frames: 1     # N프레임마다 외형 임베딩
...
viz:
  point_subsample: 4              # live point cloud 다운샘플
  frame_every: 1                  # RGB JPEG 로깅 주기
  depth_every: 1                  # depth 로깅 주기
  objects_every: 1                # object graph 로깅 주기
  trajectory_every: 1             # camera trajectory 로깅 주기
  trajectory_max_points: 0        # 0이면 전체 궤적, 양수면 최근 N점만 표시
  global_map_every: 1             # backend map 로깅 주기
  global_map_max_points: 0        # 0이면 전체, 양수면 표시용 점 수 cap
  jpeg_quality: 75                # RGB JPEG 품질. 낮추면 Rerun 부하 감소
```
...
| Rerun 로깅이 무겁다 | `viz.depth_every: 2`, `viz.objects_every: 2`, `viz.trajectory_every: 5`, `viz.global_map_max_points: 150000`, `jpeg_quality: 60` |
| YOLOE spike가 100ms를 넘긴다 | realtime profile 또는 `detect.every_n_frames: 3~5`로 검출 cadence 낮춤 |
...
# recorded OAK replay에서 tail-latency 원인 분리 매트릭스
.venv/bin/python benchmarks/perf_matrix.py sources/session_20260624_054529_194430108151D05A00 --frames 120 --out-dir artifacts/perf/matrix --headless
