...
후속 권장 (우선순위순):
1. **metric 앵커의 스케일 안정장치 승격** — calib(a,b)의 장기 drift를
   DA3METRIC 절대 스케일로 저주기 서보. 근본 원인 대응 (이전 권고 재확인).
2. 루프 검증 매칭 강화 — ORB 특징 수 상향(1200→2500+) 또는 고해상도
   매칭, inlier_dist를 장면 depth 중앙값에 비례화(고정 0.05의 스케일
   민감성 제거), min_inliers와 별도로 최소 매칭 수 게이트 분리.
3. 대형 보정의 점진 적용 — 보정 크기가 임계 초과 시 수 윈도에 걸쳐 분할.
